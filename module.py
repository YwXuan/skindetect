...
def split_Train_Val_Data(data_dir):
    ...
    train_dataloader = DataLoader(SkinDataset(train_inputs, train_labels, train_transformer),
                                  batch_size = batch_size, shuffle = True)
    test_dataloader = DataLoader(SkinDataset(test_inputs, test_labels, test_transformer),
...
C = models.googlenet(pretrained=True).to(device)     # 使用內建的 model   >>>> model = torchvision.models.vgg16(weights='IMAGENET1K_V1')
optimizer_C = optim.Adam(C.parameters (), lr=lr, betas=(0.9, 0.999), eps=1e-8, weight_decay=0.001, amsgrad=False) # 選擇你想用的 optimizer
